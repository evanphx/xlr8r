...
static NODE* specialize(NODE* scope) {
  ...
  if(nd_type(blk) != NODE_BLOCK) return blk;

  args = blk->nd_head;

  if(nd_type(args) != NODE_ARGS) return blk;
  ...
  if(args->nd_cnt == 0 && !args->nd_rest && !args->nd_opt) {
    if(nd_type(blk->nd_next) == NODE_BLOCK &&
      ...
      case NODE_NIL:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_nil;
        break;
      case NODE_TRUE:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_true;
        break;
      case NODE_FALSE:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_false;
        break;
      case NODE_SELF:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_self;
        break;
      case NODE_LIT:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_lit;
        break;
      default:
        blk->flags |= FL_FREEZE;
        blk->u2.value = (VALUE)spec_no_args;
      }
    } else {
      blk->flags |= FL_FREEZE;
      blk->u2.value = (VALUE)spec_no_args;
                 ...
                 int argc, VALUE* argv, NODE* volatile body, int flags)
{
  ...
  {
    caller c = (caller)body->nd_next->u2.value;
    result = c(klass, recv, id, oid, argc, argv, body, flags);
  } else {
    switch (nd_type(body)) {
    case NODE_CFUNC:
      {
